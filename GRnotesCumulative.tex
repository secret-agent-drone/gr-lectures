...
\begin{align}
a%\Rightarrow R^{\alpha}_{\alpha\mu\nu}&= \underbrace{g^{\alpha\lambda}}_{\text{sym }$\alpha\leftrightarrow \lambda$} R_{\lambda\alpha,\mu\nu}
\end{align}

{\Large Missing lectures above}

End of last lecture: Bianchi from contracted Riemann (Ricci)
...
\end{equation}
where $\Lambda$ is the ``cosmological'' constant (air-quotes because we do not need to discuss it in a cosmological context). This term is sometimes missing.
